...
class Decoder(nn.Module):
  def __init__(self, cn_vocab_size, emb_dim, hid_dim, n_layers, dropout, isatt):
    ...
    self.isatt = isatt
    self.attention = Attention(hid_dim*2) # diy add *2
    # 如果使用 Attention Mechanism 會使得輸入維度變化，請在這裡修改
    # e.g. Attention 接在輸入後面會使得維度變化，所以輸入維度改為
    self.input_dim = hid_dim * 2 if isatt else emb_dim
...
class Attention(nn.Module):
  def __init__(self, hid_dim):
    ...
    self.attention_network = nn.Sequential(
      nn.Linear(in_features=self.hid_dim+config.emb_dim,out_features=config.max_output_len),
      nn.Softmax(dim=2),
    )
    self.attn_cat = nn.Sequential(
      nn.Linear(self.hid_dim + config.emb_dim, self.hid_dim),
  ...
  def forward(self, encoder_outputs, decoder_hidden,decoder_input):
    ...
    attn = torch.cat((decoder_input,decoder_hidden[-1].unsqueeze(1)),dim=2) 
    attn_weights = self.attention_network(attn) 
    attn_applied = torch.bmm(attn_weights,encoder_outputs)
    att_output = torch.cat((decoder_input, attn_applied), dim=2)
    att_caten = self.attn_cat(att_output)
    att_scaled = F.softmax(att_caten) #to (0, 1)
    return att_scaled
...
class Seq2Seq(nn.Module):
  ...
  def inference_beam(self, input, target):
    ########
    # TODO #
    ########
    # 在這裡實施 Beam Search
    ...
    # 準備一個儲存空間來儲存輸出
    outputs = torch.zeros(batch_size, input_len, vocab_size*2).to(self.device)
    # 將輸入放入 Encoder
    encoder_outputs, hidden = self.encoder(input)
    # Encoder 最後的隱藏層(hidden state) 用來初始化 Decoder
    # encoder_outputs 主要是使用在 Attention
    ...
    input = target[:, 0]
    preds = []
    print('input should be like ',input)
    output, hidden = self.decoder(input, hidden, encoder_outputs)
      # 將預測結果存起來
    print('output to be appended should be like ', output.shape)
    outputs[:, 1] = torch.cat((output, output), 1)
    print('output to be appended is ', outputs[:,1].shape)
    print('hidden should be like ', hidden.shape)
    # print(output.argmax(1))
    # print(torch.topk(output, k = 1))
    # 取出機率最大的單詞
    top1 = torch.topk(output, k = 2).indices
    input = top1
    preds.append(top1)
    # print('input is ',input)
    # print('input0 is ',torch.tensor([input[0][0]]))
    # print('input1 is ',torch.tensor([input[0][1]]))
    # input = torch.tensor([torch.tensor([input[0][0]]), torch.tensor([input[0][1]])])
    # print(input[-1])
    # print(input[0])
    output1, hidden1 = self.decoder(torch.tensor([input[0][0]]).cuda(), hidden, encoder_outputs)
    # print('output1[0][0] is ', output1[0][0])
    print('hidden1 be like ', hidden1.shape)
    output2, hidden2 = self.decoder(torch.tensor([input[0][1]]).cuda(), hidden, encoder_outputs)
    print('hidden2 be like ', hidden2.shape)
    length = output2.shape
    length = int(length[1])
    # print('length is ', length[1])
    # print('output2[0][0] is ', output2[0][0])
    output = torch.cat((output1, output2), 1)
    # print('torch.cat((output1, output2), 1)', torch.cat((output1, output2), 1))
    # top1 = torch.topk(output1, k = 2)
    # top2 = torch.topk(output2, k = 2)
    # print('top1 is ', top1)
    # print('top2 is ', top2)
    top = torch.topk(output, k= 2).indices
    # print(top)
    input = torch.squeeze(top, 0)
    print('input is ', input)
    # input2 = top2.indices
    # print('input1 is ',input1)
    # print('input2 is ',input2)
    hidden = []
    # To decide whether input1 is with hidden1 or hidden2
    if int(input[0]) >= length :
      hidden.append(hidden1)
    else:
      hidden.append(hidden2)
    if int(input[1]) >= length :
      hidden.append(hidden1)
    else:
      hidden.append(hidden2)
    # top = torch.cat((top1, top2))
    print('hidden[0] be like ', hidden[0].shape)
    print('hidden[1] be like ', hidden[1].shape)
    print('pass')
    for t in range(3, input_len-1):
      output1, hidden1 = self.decoder(torch.tensor([input[0]]).cuda(), hidden[0], encoder_outputs)
      output2, hidden2 = self.decoder(torch.tensor([input[1]]).cuda(), hidden[1], encoder_outputs)
      output = torch.cat((output1, output2), 1)
      outputs[:, t] = output
      # 取出機率最大的單詞
      top = torch.topk(output, k= 2).indices
      ...
      if int(input[1]) >= length :
        ...
        hidden.append(hidden2)
      # print('top1.unsqueeze(1) is ',top1.unsqueeze(1))
      # print(type(top))
      preds.append(top)
    ...
    output2, hidden2 = self.decoder(torch.tensor([input[1]]).cuda(), hidden[1], encoder_outputs)
    output = torch.cat((output1, output2), 1)
    outputs[:, input_len-1] = output
    top = torch.topk(output, k= 1).indices
    preds.append(top) 
    preds = torch.cat(preds, 1)
    print('preds is like ', preds.shape)
    return outputs, preds
...
class configurations(object):
  def __init__(self):
    ...
    self.store_model_path = "./ckpts/cktp_" + time.strftime("%Y%m%d-%H%M%S")     # 儲存模型的位置
    ...
    self.beam_search_K = 2
    # os.makedirs(self.store_model_path)
  ...
  def save_to_file(self):
    data = {
      ...
      "store_model_path": self.store_model_path,
      "load_model_path": self.load_model_path,
      "data_path": self.data_path,
      "attention": self.attention
    }
    with open(os.path.join(self.store_model_path,"config.json"), 'w') as outfile:
...
#   np.save(os.path.join(config.store_model_path,"train_losses.npy"),train_losses)
#   np.save(os.path.join(config.store_model_path,"val_losses.npy"),val_losses)
#   np.save(os.path.join(config.store_model_path,"bleu_scores.npy"),bleu_scores)
"""## test"""
...
if __name__ == '__main__':
  ...
  config.load_model_path = "model_git_drop.ckpt"#sys.argv[1]
  print ('config:\n', vars(config))
